...
- 2026-06-09 因 `PlatformProfile` 安全区 / 震动边界护栏重新导出 `exports/android/NightRunner-debug.apk`；当前 APK `28,416,550` bytes，签名验证通过 v2/v3
...
- 2026-06-09 `PlatformProfile` 安全区 / 震动边界护栏后再次完成 `NightRunner35` 模拟器安装启动验证：`adb install -r` 成功，`monkey` 可拉起应用，`pidof` 返回进程 `5423`，`dumpsys activity` 显示 `GodotAppLauncher` 为 top resumed activity
...
- 当前机器上的 `adb`、模拟器和 AVD 已验证可用；`platform-tools` 当前为 `37.0.0`，如果 `adb devices` 无在线设备，需先启动 `NightRunner35` 或连接真机，再做安装启动验证
