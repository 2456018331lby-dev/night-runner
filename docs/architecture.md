...
- 安全区 margin 计算和震动支持边界并入 `verify_settings.tscn` 覆盖；改安全区、移动端 UI scale、平台判断或震动入口后先跑它
...
- 移动端 `HUD` 会隐藏桌面版 `PhaseCard` / `SecondaryCard`，但导航卡必须保留路线压力和可选目标短状态；改移动端 HUD 密度或导航文案后先跑 `verify_dynamic_health_hud.tscn`
