...
- 局前构筑面板会持续展示 directive 的 summary 和 modifier 影响，选择新 directive 后不会丢掉收益 / 风险文本，按钮的 `ACTIVE` / `OPTION` 状态也会同步更新。
...
- 收敛 `SessionScreen` 的 directive 详情和按钮状态格式，复用已有 `verify_pause_settings.tscn` 覆盖局前 modifier 文本与 `ACTIVE` / `OPTION` 切换，避免再新增单点测试文件。
